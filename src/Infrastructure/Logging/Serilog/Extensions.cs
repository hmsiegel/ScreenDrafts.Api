namespace ScreenDrafts.Api.Infrastructure.Logging.Serilog;
public static class Extensions
{
    ...
    private static void ConfigureElasticSearch(WebApplicationBuilder builder, LoggerConfiguration serilogConfig, string appName, string elasticSearchUrl)
    {
        ...
        {
            string? formattedAppName = appName?.ToLower().Replace(".", "-", StringComparison.CurrentCultureIgnoreCase).Replace(" ", "-", StringComparison.CurrentCultureIgnoreCase);
            string indexFormat = $"{formattedAppName}-logs-{builder.Environment.EnvironmentName?.ToLower().Replace(".", "-", StringComparison.CurrentCultureIgnoreCase)}-{DateTime.UtcNow:yyyy-MM}";
