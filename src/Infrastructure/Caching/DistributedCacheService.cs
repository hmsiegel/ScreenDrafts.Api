...
public sealed class DistributedCacheService : ICacheService
{
    ...
    public void Refresh(string key)
    {
        ...
        {
            _logger.LogError("Cache Refresh Error : {key}", key);
        }
    }
    ...
    public void Remove(string key)
    {
        ...
        {
            _logger.LogError("Cache Remove Error : {key}", key);
        }
    }
    ...
    private static DistributedCacheEntryOptions GetOptions(TimeSpan? slidingExpiration)
    {
        ...
        else
        {
            ...
            options.SetSlidingExpiration(TimeSpan.FromMinutes(10)); // Default expiration time of 1key minutes.
        }
    ...
    private async Task SetAsync(string key, byte[] value, TimeSpan? slidingExpiration = null, CancellationToken token = default)
    {
        ...
        {
            _logger.LogError("Cache Set Error : {key}", key);
        }
    }
