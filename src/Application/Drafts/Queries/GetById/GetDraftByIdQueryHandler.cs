...
internal sealed class GetDraftByIdQueryHandler : IQueryHandler<GetDraftByIdQuery, DraftResponse>
{
    ...
    public GetDraftByIdQueryHandler(
    ...
    public async Task<Result<DraftResponse>> Handle(GetDraftByIdQuery request, CancellationToken cancellationToken)
    {
        var draft = await _draftRepository.GetByIdAsync(DefaultIdType.Parse(request.Id), cancellationToken);

        if (draft is null)
        {
            return Result.Failure<DraftResponse>(DomainErrors.Draft.NotFound);
        }
        ...
        foreach (var hostId in draft.HostIds!)
        {
            ...
            if (host is null)
            {
                return Result.Failure<DraftResponse>(DomainErrors.Host.NotFound);
            }
